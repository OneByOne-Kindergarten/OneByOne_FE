 ...
 */
export async function sendToFlutter<T = unknown, R = any>(type: MessageType | string, data: T): Promise<R> {
  ...
  try {
    // 핸들러에서 반환한 값을 직접 반환
    const result = await window.flutter_inappwebview.callHandler('onWebViewMessage', JSON.stringify(message));
    return result || { status: 'error', message: 'No response from Flutter' } as unknown as R;
  } catch (error) {
 ...
 */
export function listenToFlutter<T = unknown>(type: MessageType | string, callback: MessageCallback<T>): () => void {
  ...
  // 구독 취소 함수 반환
  return () => {
    const index = messageCallbacks[type]?.indexOf(callback as MessageCallback) ?? -1;
    if (index !== -1) {
      messageCallbacks[type]?.splice(index, 1);
    }
    
    // 모든 콜백이 제거되면 이벤트 리스너도 제거
    if (Object.values(messageCallbacks).every(callbacks => callbacks.length === 0)) {
