...
export function useRequestFcmToken(): [
  ...
  const requestFcmToken = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const result = await sendToFlutter<{}, { status: string; token?: string; message?: string }>(
      ...
      if (result?.status === 'success' && result.token) {
        ...
        const errorMsg = result?.message || 'FCM 토큰을 가져오는데 실패했습니다.';
        setError(errorMsg);
        return null;
      }
    } catch (error) {
...
export function usePermission(permissionType: PermissionType): [
  ...
  const requestPermission = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const result = await sendToFlutter<{type: PermissionType}, PermissionResult>(
